...
# =========================
# CSS THEME (NỀN CŨ & NÚT TRÒN & XÓA KHUNG CHỮ NHẬT)
# =========================
custom_css = """
...
body, html { 
    ...
    background-image: 
        radial-gradient(circle at 15% 50%, rgba(45, 212, 191, 0.2), transparent 40%), 
        radial-gradient(circle at 85% 30%, rgba(59, 130, 246, 0.2), transparent 40%) !important;
...
#root, .gradio-container, .main, .wrap {
...
.status-tracker, .load-status, .progress-text, .eta-bar, .generating-text, div[data-testid="status-tracker"] {
...
.hero-title { font-size: 2.8rem; font-weight: 700; color: #ffffff; text-align: center; margin-bottom: 35px; letter-spacing: -0.5px; }
...
/* FIX: Tắt hoàn toàn màu nền của các khối Gradio tạo ra bên trong input-row */
.input-row { 
...
.input-row div[data-testid="textbox"] {
    background: transparent !important;
    background-color: transparent !important;
    border: none !important;
    box-shadow: none !important;
...
.input-row textarea::placeholder { color: #565869 !important; }
...
.laptop-card.expanded .view-detail-btn:hover { background: var(--accent) !important; color: #ffffff !important; }
...
with gr.Blocks() as interface:
    ...
    current_results = gr.State(value=[]) 
    ...
    with gr.Row(elem_id="main-container"):
        ...
        with gr.Column(elem_classes=["chat-col", "center"], elem_id="chat-col") as chat_col:
            ...
            hero_title = gr.HTML("<div class='hero-title'>How can I help?</div>", visible=True)
    ...
    sub_evt = submit_btn.click(
        fn=chat_logic, 
        inputs=[query_input, session_memory, chat_history, chatbot_ui, current_results], 
        outputs=[output_display, session_memory, chat_history, chatbot_ui, current_results, query_input, results_col, chat_col, hero_title, chatbot_ui, clear_btn]
    )
